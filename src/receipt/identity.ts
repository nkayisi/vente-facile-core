...
/**
 * `loadLogo` ne vit PAS ici.
 *
 * Charger une image et la réduire en dataURL passe par le DOM sur le web
 * (`Image` + `canvas`) et par le système de fichiers sur mobile : deux
 * implémentations irréconciliables. Ce paquet ne décrit que le document, il
 * reçoit donc un `LoadedLogo` déjà construit par la surface appelante.
 *
 *   web    : frontend/lib/receipt/load-logo.ts
 *   mobile : src/printing/load-logo.ts
 */
